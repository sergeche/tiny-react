...
const supportedEvents = [
    ...
    'onended', 'onloadeddata', 'onloadedmetadata',
...
module.exports = function(tree) {
    ...
    return node => {
        ...
        // remove event properties from generated DOM nodes
        // XXX maybe remove events in runtime during capturing phase?
        var allNodes = Array.from(node.getElementsByTagName('*'));
        allNodes.push(node);
        allNodes.forEach(n => {
...
/**
 * Fast check for possible candidates: it’s faster to test if given key
 * looks like `onSomething` instead of lower-casing all prop names
 * @param  {String} name
 * @return {Boolean}
 */
